...
from evo import transformations as tr
from evo.pose_evaluation_utils import quat_pose_to_mat


class kittiOdomEval():
    def __init__(self, gt_pose_path, result_path, eval_seqs=None):
        assert os.path.exists(gt_pose_path), "Error of ground_truth pose path!"
        gt_files = glob.glob(gt_pose_path + '/*.txt')
        gt_files = [os.path.split(f)[1] for f in gt_files]
        self.seqs_with_gt = [os.path.splitext(f)[0] for f in gt_files]
        self.seqs_with_gt.sort(key=lambda x: int(x))

        if eval_seqs is None:
            self.seqs_with_gt = ['00', '01', '02', '03', '04', '05', '06', '07', '08', '09', '10']

        self.lengths = [100, 200, 300, 400, 500, 600, 700, 800]
        self.num_lengths = len(self.lengths)
        self.gt_dir = gt_pose_path
    ...
    def eval(self, toCameraCoord):
        ...
        for seq in self.eval_seqs:
            ...
            pred_file_name = self.result_dir + '/pred_pose_{}.txt'.format(seq)
            ...
            print("T / R : {0:.4f}".format(ave_t_err * 100),"/ {0:.4f}\n".format(ave_r_err / np.pi * 180 * 100))
            with open(eva_seq_dir + '/%s_stats.txt' % seq, 'w') as f:
                f.writelines('Average sequence translation RMSE (%):    {0:.4f}\n'.format(ave_t_err * 100))
                f.writelines('Average sequence rotation error (deg/100m):  {0:.4f}'.format(ave_r_err / np.pi * 180 * 100))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gt_dir', type=str, default='../dataset/kitti/poses',
